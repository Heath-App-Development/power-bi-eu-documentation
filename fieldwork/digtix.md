...
As noted in the introduction, the information in this report is **not** real-time.  Early every morning DigTix sends us a file that has the information for every ticket that's currently in their system as of the time that they generated the file.  On our end we pick up the file, automatically process it, and put the results into a place that the report refreshes from.  The report refresh only happens once a day, at 6:30 a.m. Central time.
